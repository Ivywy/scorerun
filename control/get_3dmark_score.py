...
def get_3dmark_log(rootDir,dstPath, app):
	...
	fileList = list()
	if app == "TimeSpy":
		fileList=[f for f in os.listdir(rootDir) if os.path.isfile(os.path.join(rootDir,f)) and f.endswith('.3dmark-result') and f.__contains__("TimeSpyExtremeCustom") and not f.__contains__("FAILED")]
	elif app == "TimeSpy_FPS":
		fileList=[f for f in os.listdir(rootDir) if os.path.isfile(os.path.join(rootDir,f)) and f.endswith('.3dmark-result') and f.__contains__("TimeSpyExtremeCustom") and not f.__contains__("FAILED")]
	elif app == "FireStrike":
		fileList=[f for f in os.listdir(rootDir) if os.path.isfile(os.path.join(rootDir,f)) and f.endswith('.3dmark-result') and f.__contains__("FireStrikeCustom") and not f.__contains__("FAILED")]
